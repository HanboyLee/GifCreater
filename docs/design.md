...
### 1. 痛点剖析与 TypeSafe 决策
针对文生图模型原生无 Alpha 通道导致的“无法制作透明表情包”及全图注意力机制导致的“角色肢体/物品超出网格跨格穿模”痛点，经 TypeSafe Jev（置信度 1.0）裁决：
1. **背景模式三态显式可控 (Background Modes Tri-state)**：
   - **纯色透明底 (Transparent / Solid White)**：强制指定 `#FFFFFF` 纯净平铺底色、独立贴纸主体 (Sticker Style)、无地面投影与环境杂质，切片后可完美作为微信透明表情包或极速去底；
   - **场景环境 (Continuous Scenic)**：强制各分镜共用统一且连贯的场景环境与自然光影；
   - **自由不限 (Auto)**：保持中立通用无框约束。
...
3. **大模型动作就地收敛 (Locally Contained Action Prompting)**：
   - 系统提示词限制所有动作和特效收拢于角色近身区域，严禁横向大位移与穿屏拖尾。

---

## 十九、 变更与演进记录 (Changelog)
