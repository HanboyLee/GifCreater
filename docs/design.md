...
## 十七、 Prompt 工作台异步「完善」四重状态反馈机制设计 (Compound Fluent Feedback Architecture)

### 1. 痛点剖析与 TypeSafe 决策
针对大模型生成耗时（通常 3~15 秒）造成的“无从得知运行状态、是否卡死或失败”的痛点，经 TypeSafe Jev（置信度 1.0）裁决，采用组合式 Fluent 即时状态反馈：
1. **按钮态变 (Button State Transition)**：点击后文字瞬间切为 `⏳ 正在完善...`，并进入禁用态防抖；响应后立即复原；
2. **原生 Fluent 流光进度条 (`IndeterminateProgressBar`)**：在输入框正下方展开动态呼吸流光动画，直观传递网络运转态；
3. **就地常驻状态提示行 (`CaptionLabel`)**：
   - 运行中：显式呈现当前调用模型名称 `⏳ 正在调用大模型 ({model_id}) 生成分镜描述...`；
   - 成功态：呈现绿色 `✓ 分镜描述完善成功，已更新至下方编辑器`；
   - 失败态：呈现深浅自适应红色 `❌ 完善失败: {具体错误}`，就地常驻，绝不因弹窗消失而丢失线索；
   - 未配 Key：直接黄色提示 `⚠️ 请先在「设置」中配置 API Key`；
4. **双保险系统通知**：完成或失败时同步触发右上角 `InfoBar`。

## 十八、 分镜提示词背景模式与防越界穿模架构设计 (Background Modes & Anti-Bleed Architecture)

### 1. 痛点剖析与概念解耦
针对“无法制作透明表情包”、“纯色与透明概念混淆”及扩散模型“角色肢体/物品超出网格跨格穿模”痛点，执行概念彻底解耦与护城河体系：
1. **背景模式四态解耦与显式可控 (Decoupled Background Modes)**：
   - **透明背景 (Transparent, 表情包推荐)**：强制指定 `completely transparent background`、独立贴纸主体 (Die-cut Sticker Style)、无地面投影与环境杂质，切片后可作为微信透明免抠表情包；
   - **纯色背景 (Solid Color)**：强制指定 `#FFFFFF` 纯色平铺底色 (`pure solid flat white background`)，纯色无渐变无杂质，不透明；
   - **场景背景 (Continuous Scenic)**：强制各分镜共用统一且连贯的场景环境、道具与自然光影；
   - **自由不限 (Auto)**：保持中立通用无框约束，交由模型自适应推断。
2. **防穿模物理约束锁 (Anti-Bleed Hard Constraints)**：
   - **紧凑角色比例 (Compact Scale)**：强制角色在单格内仅占 **60%~70%** 的宽高，留足 30%~40% 的宽裕安全空域；
   - **15% 安全隔离护城河 (Safety Margin)**：强制每格四周预留至少 **15%** 的单格宽度空白缓冲带；
   - **绝对防溢出负向约束**：显式禁止任何肢体、毛发、尾巴、武器、道具或魔法粒子光效跨越格子边界。
3. **大模型动作就地收敛 (Locally Contained Action Prompting)**：
   - 系统提示词限制所有动作和特效收拢于角色近身区域，严禁横向大位移与穿屏拖尾；透明底模式下严禁描绘环境道具与影子。

## 十九、 GIF 格式透明通道全链路保留与边缘抗光晕量化架构设计 (Transparent GIF & Anti-Halo Architecture)

### 1. 痛点根因与技术挑战
1. **透明变黑缺陷**：传统实现将 RGBA 帧在进入 GIF 编码器前强制调用 `.convert("RGB")`，由于 RGB 没有 Alpha 通道，PIL 默认将透明像素置为黑色 `(0, 0, 0)`，且未指定 GIF `transparency` 与 `disposal` 参数；
2. **边缘光晕与毛刺**：PNG 原图具备 8-bit Alpha 平滑羽化抗锯齿边缘（半透明），而 GIF89a 仅支持 1-bit 二进制透明度。半透明像素在未经净化的状态下被默认底色污染（与黑色预乘），导致人物四周出现杂色黑圈与狗牙毛刺；
3. **多帧时序残影溢出**：若未显式指定 `disposal=2`（Restore to Background），在透明背景下上一帧的角色像素会残留在当前帧的背景区域中，造成多帧串位。

### 2. 核心算法设计 (`quantize_frames_for_gif`)
1. **全序列采样蒙太奇大图与统一全局调色板**：
   从序列中均匀抽取具有代表性的帧生成蒙太奇样本，使用中位切分法（MEDIANCUT）量化出全局统一调色板，杜绝多帧调色板不一致造成的闪烁或色偏；
2. **专属透明色槽位预留**：
   量化色彩数设为 `safe_colors = min(colors, 256) - 1`，将第 `safe_colors` 号索引专属保留给透明色（`trans_idx`）；
3. **半透明二值化清洗与底色纯化 (Defringing & Matte Purification)**：
   对 Alpha 通道以阈值 128 执行二值化；将 Alpha < 128 的像素强制重置为纯净透明，Alpha >= 128 的像素保留真实色彩并映射至调色板，彻底消除暗黑光晕；
4. **时序无残留刷新与元数据绑定**：
...
## 二十一、 动态 Provider Models API 接入与搜索锁定设计 (Dynamic Remote Models Sync & Lock)

### 1. 痛点剖析与 TypeSafe Jev 决策
- **痛点**：原有设置页仅预设了 10 个固定静态模型，OpenRouter、OpenAI 等平台成百上千款新模型无法被搜索与选择，且输入自定义模型时缺乏直观的选中锁定反馈。
- **决策结论**：经 TypeSafe Jev（置信度 1.0）裁决采用 `fetch_cache_lock` 架构：动态对接各 Provider 官方 `GET /models` 标准接口，辅以本地文件缓存与 `QThread` 异步非阻塞调度，实现全量模型即搜即选并精准锁定。

### 2. 核心架构与交互链路
1. **统一端点协议适配 (`fetch_remote_models`)**：
   - 自动适配规范化端点：`{base_url.rstrip('/')}/models`；
   - 支持 Bearer Token 认证头传递；
   - 健壮解析规范的 `{"data": [{"id": "model_name"}, ...]}` 结构，提取模型 ID 列表并执行字典序去重整理。
2. **本地文件级缓存与热重载 (`models_cache.json`)**：
   - 成功拉取的模型列表自动归档至本地输出目录的 `models_cache.json`；
   - 应用冷启动或切换 Provider 时，优先毫秒级加载本地缓存，彻底消除等待；
   - 缓存未命中时优雅回退至默认内置预设。
3. **异步非阻塞 Worker (`ModelFetchWorker`)**：
   - 采用 `QThread` 封装网络 I/O，设置 8 秒弹性超时，UI 界面保持 60 FPS 流畅；
   - 提供「🔄 获取最新模型」操作，触发后按钮态变为「⏳ 正在获取...」防抖，成功/失败通过 Fluent InfoBar 给出明确状态反馈。
4. **实时模糊过滤与选中锁定机制**：
   - 在搜索框中输入任意关键词实时高亮过滤下拉列表；
...
- **2026-09-20 (Prompt 工作台「完善」异步状态多维反馈升级)**：依据 TypeSafe Jev 100% 裁决，设计并落地按钮态变、原生 Fluent 流光进度条、就地模型感知与常驻错误回显四重反馈机制，彻底解决黑盒等待体验痛点。
- **2026-09-20 (v3.5 体验修补)**：完善 Prompt 改为代码锁定无网格线 sprite sheet；切片 smart_crop 按格尺寸去缝；均匀等分关闭去缝开关；配文增加字号。
- **2026-09-18 (v3.5 UI 拍板)**：三页导航、Prompt 工作台、设置主题横排与 API 持久化边界写入设计；废除 MJ 语法与侧栏收藏卡方案。
