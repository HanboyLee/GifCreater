...
license: MIT
description: >
  MANDATORY skill for all technical planning, architecture design, solution proposals,
  and implementation plans (implementation_plan.md). Use this skill whenever the user asks
  for a plan, feature proposal, architecture trade-off, or code design, or when deciding
  between pure code vs generative models vs structured judgments (Choice/Noul/Score).
  Guides the agent to read live docs (https://docs.typesafe.ai/llms.txt), work backward from
  deliverables, keep rules and computation in code, and use System One primitives (Choice/Noul/Score)
  for semantic decisions instead of fragile prompt-and-parse.
---
