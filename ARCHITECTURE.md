...
* **职责**：
  ...
  * (v3.5+) 提示词结构化数据契约与 Agent 规范生成引擎 (`prompt_schema.py` / `agent_engine.py`，支持纯色透明/场景连贯/自由三态背景与 60%~70% 紧凑尺度 + 15% 边距防越界穿模硬约束)。
...
* **职责**：
  ...
  * (v3.5) Fluent 导航三页：动图工坊、Prompt 工作台（黄金 4 强快捷网格 3×3/4×4/4×6/6×4、背景三态单选切换、完善状态实时指示条与状态文本）、设置（主题横排 + API；主题/API 持久化）。
