...
* **职责**：
  * 多帧拼图切片与边缘探测算法 (`slicer.py`，支持基于投影能量积分波谷的自适应槽隙中位线中心吸附，消除分镜错位)；
  * 微信表情包 ≤500KB 调色板自适应搜索与试探压缩 (`compressor.py`，支持 RGBA 全局共享调色板量化、透明通道槽位保留、半透明二值化边缘抗光晕与 disposal=2 无残留时序刷新，100% 尊重配置帧间隔与尾帧停顿)；
  * GIF / WebP 动图合成与帧延时编码 (`exporter.py`，全格式原生支持透明背景、边缘净化、Boomerang 往复镜像展开与时序保真编码)；
  * 表情包自由排版与文字渲染引擎 (`caption.py`，支持任意坐标比例、-180°~180°旋转、文本/描边 RGBA 双重着色与透明度、自适应描边与字体优雅回退)；
  * (v3.5+) 提示词结构化数据契约与 Agent 规范生成引擎 (`prompt_schema.py` / `agent_engine.py`，支持透明免抠/纯色平铺/场景连贯/自由四态背景与 60%~70% 紧凑尺度 + 15% 边距防越界穿模硬约束)。

### 2. 交互表现层 (`src/gifcreater/ui/`)
* **准则**：专注于视图呈现与用户输入，**严禁在主线程执行耗时计算**。遵守 `UIUX-PRO-MAX` 规范。
* **职责**：
  * 现代 Fluent 窗口框架、自适应深浅色主题渲染；
  * QGraphicsView 交互画布（支持滚轮缩放、平移与网格虚线拖拽，内置 `DraggableCaptionItem` 实时配文所见即所得图层，支持鼠标直接拖拽排版文字）；
  ...
  * (v3.5) Fluent 导航三页：动图工坊、Prompt 工作台（黄金 4 强快捷网格 3×3/4×4/4×6/6×4、背景四态解耦单选切换、完善状态实时指示条与状态文本）、设置（主题横排 + API；主题/API 持久化）。
